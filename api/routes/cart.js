...
module.exports = () => {
  ...
  router.get("/", function (req, res) {
    routeWrapper(req, res, true, (token) =>
      new CartModel().getCartData({ ...req.query, user_id: token.data.id }).then((data) => ({ success: true, data: data }))
    );
  });
  ...
  router.get("/pgs", function (req, res) {
    routeWrapper(req, res, true, () => new PaymentGatewayModel().list({ where: { is_active: true } }));
  });
  ...
  router.get("/orderSuccess/:id", function (req, res) {
    routeWrapper(req, res, true, (token) => {
      return new PaymentModel()
        .getOrdertData({ id: req.params.id, user_id: token.data.id })
        .then((data) => {
