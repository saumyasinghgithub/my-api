...
class CourseModel extends BaseModel {
  ...
  getByTrainer(tid) {
    return this.list({ where: { user_id: tid }, limit: 9999999, sortBy: "created_at", sortDir: "DESC" })
      .then(({ data }) => {
        return this.getCoursesWithResources(data);
      })
      .then((courses) => {
        return this.fetchRatings(courses.map((c) => c.id)).then((ratings) => {
  ...
  markfav({ user_id, course_id, fav }) {
    return new Promise((resolve, reject) => {
      this.db
        .run(`DELETE FROM favorite_courses WHERE user_id=? AND course_id=?`, [user_id, course_id])
        .then(() => {
          if (parseInt(fav) === 1) {
...
class CourseRating extends BaseModel {
  ...
  getRatingByCourses(cids) {
    ...
    return new Promise((resolve, reject) => {
      this.db
        .run(
          "SELECT course_id,AVG(rating) as rating,COUNT(id) as ratings FROM  " +
            this.table +
            " WHERE course_id IN (" +
            cids.join(",") +
            ") GROUP BY course_id"
  ...
  getRatingByCourse(course_id) {
    return this.db
      .run("SELECT AVG(rating) as rating,COUNT(user_id) as ratings FROM " + this.table + " WHERE course_id=?", [course_id])
      .then((res) => ({
  ...
  save(ratingObj) {
    return this.deleteWhere({ user_id: ratingObj.user_id, course_id: ratingObj.course_id }).then(() => this.add(ratingObj));
  }
}
